...
<template>
  <section v-if="attendees.length > 0" class="mt-8">
    <h2 class="text-lg font-semibold text-gray-900 dark:text-white">Your tickets</h2>
    <p class="mt-1 text-sm text-gray-600 dark:text-gray-300">Show these at the door for check-in.</p>
    ...
    <div class="mt-4 grid grid-cols-2 gap-4 sm:grid-cols-3">
      ...
      >
        <img :src="qrSrc(a.id)" :alt="`Ticket QR for attendee #${a.id}`" width="180" height="180" loading="lazy" />
      </figure>
    </div>
  </section>
</template>
